...
import TermsStyles from "../styles/MainStyles"; // Importa los estilos

const TermsScreen: React.FC = () => {
  const { isDarkMode } = useTheme(); // Obtiene el estado del modo oscuro

  return (
    <View style={[TermsStyles.container, { backgroundColor: isDarkMode ? "#000" : "#fff" }]}>
      <ScrollView contentContainerStyle={TermsStyles.scrollContainer}>
        {/* Título centrado */}
        <Text style={[TermsStyles.title, { color: isDarkMode ? "#fff" : "#000" }]}>
          Terms and Conditions
        </Text>

        {/* Contenido de términos y condiciones */}
        <Text style={[TermsStyles.text, { color: isDarkMode ? "#aaa" : "#333" }]}>
